 ...
 * passing untouched.
 *
 * `isEimzoMock()` is the switch itself: `VITE_EIMZO_MOCK` unset or anything
 * other than the literal string `'true'` means real mode — which is how dev
 * and prod are configured today and stay (`LoginPage.tsx`'s own existing
 * `import.meta.env.VITE_EIMZO_MOCK === 'true'` check, task 10's five call
 * sites now match it exactly). Deliberately a FUNCTION, not a module-level
 * constant: `import.meta.env` is a live object Vite/vitest mutate in place
 * (`vi.stubEnv`), but a top-level `const` would capture whatever value was
 * current at first import and never see a later override — exactly what
 * `LoginPage.test.tsx`'s existing "mock flag is off" test does mid-suite,
 * after every module in this file is already loaded.
 *
...
export function isEimzoMock(): boolean {
  return import.meta.env.VITE_EIMZO_MOCK === 'true';
}
