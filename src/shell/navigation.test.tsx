...
/**
 * The superuser the way the live `/auth/me` actually answers for `sys_admin`:
 * `is_superuser: true` AND every code in the registry (`auth/router.py`:
 * `sorted(PERMISSIONS) if is_superuser`) — 58 of them on the dev stand,
 * `applications.create` among them. A fixture with `permissions: []` let a
 * gate that checked "is the code held?" pass its tests and still open the
 * citizen's cabinet to `admin` on the stand (2026-09-10, PR #56).
 */
const SUPERUSER = { permissions: ['applications.create', 'applications.review'], is_superuser: true };

test('the superuser sees every staff screen, including items granted to nobody', () => {
  const items = visibleNav(SUPERUSER);
...
test('a noSuperuser gate refuses the superuser even though /auth/me hands them every code', () => {
  ...
  // Without the flag, the bypass stays what it is everywhere else.
  expect(satisfies('applications.create', SUPERUSER)).toBe(true);
  expect(satisfies('payments.recipients.manage', { permissions: [], is_superuser: true })).toBe(true);
});
...
test("every gated navigation entry's route requires the same permission NAVIGATION declares", () => {
  ...
  for (const item of NAVIGATION) {
    ...
    const element = route?.element as { props?: { permission?: string; noSuperuser?: boolean } } | undefined;
    expect(element?.props?.permission).toEqual(item.permission);
    // The superuser exclusion rides along the same way: declared once on
    // the entry, read back off the route's own element.
    expect(element?.props?.noSuperuser).toEqual(item.noSuperuser);
  }
});
