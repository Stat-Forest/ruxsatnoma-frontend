...
export function ProfilePage() {
  ...
  return (
    <div data-testid="profile-page" className="max-w-4xl mx-auto space-y-6 pb-12">
      {/* Hero Banner Card */}
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#123522] via-[#1E5638] to-[#2E7D4F] text-white p-6 sm:p-8 shadow-md">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-6">
          ...
          <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-white/15 backdrop-blur-md border border-white/25 shadow-inner flex items-center justify-center text-2xl sm:text-3xl font-extrabold text-white tracking-wider shrink-0 ring-4 ring-white/10">
            {getInitials(me?.user.full_name)}
          </div>

          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-white/20 text-white backdrop-blur-sm border border-white/25 shadow-xs">
                <Shield className="w-3.5 h-3.5 text-emerald-300" />
                {roleName}
              </span>
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-400/20 text-emerald-200 border border-emerald-400/30">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                {lang === 'ru' ? 'Активный аккаунт' : lang === 'uz_cyrl' ? 'Фаол ҳисоб' : lang === 'en' ? 'Active account' : lang === 'kaa' ? 'Aktiv esap' : 'Faol hisob'}
              </span>
              {me?.is_superuser && (
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-400/25 text-amber-200 border border-amber-400/40">
                  Superuser
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs text-emerald-100/90">
              <div className="flex items-center gap-1.5 bg-black/20 backdrop-blur-xs px-2.5 py-1 rounded-lg border border-white/10">
                ...
                <span className="font-mono text-white">{me?.user.login ?? '—'}</span>
              </div>
              <div className="flex items-center gap-1.5 bg-black/20 backdrop-blur-xs px-2.5 py-1 rounded-lg border border-white/10">
      ...
      <div className="bg-white border border-[#E4E7EA] rounded-2xl p-1.5 shadow-xs overflow-x-auto">
        <nav className="flex gap-1.5 min-w-max" aria-label="Tabs">
          {[
            ...
            return (
              ...
              >
                <Icon className={`w-4 h-4 ${isActive ? 'text-white' : 'text-[#767F87]'}`} />
      ...
      {tab === 'password' && (
        <section className="bg-white border border-[#E4E7EA] rounded-2xl p-6 sm:p-7 shadow-xs">
          <div className="mb-6 pb-4 border-b border-[#E4E7EA]">
            <h2 className="text-base sm:text-lg font-bold text-[#1A1F24] flex items-center gap-2">
              <Lock className="w-5 h-5 text-[#2E7D4F]" />
              {passwordLabels.title}
            </h2>
