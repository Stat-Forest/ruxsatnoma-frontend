...
test('the outbox Excel button asks the server for the export with the applied filters, never paging the list itself', async () => {
  mockBackend({ outbox: [outboxMessage()] });
  let exportUrl: URL | null = null;
  server.use(
    http.get('*/api/v1/admin/integrations/outbox/export.xlsx', ({ request }) => {
      exportUrl = new URL(request.url);
      return HttpResponse.text('xlsx-bytes', {
        headers: {
          'Content-Disposition': 'attachment; filename="chiquvchi-navbat-2026-09-11.xlsx"',
          'X-Export-Total': '1',
          'X-Export-Rows': '1',
  ...
  await user.click(screen.getByRole('button', { name: 'Qoʻllash' }));
  await screen.findByTestId(`outbox-row-${DEAD_MESSAGE}`);
  const outboxCallsBefore = calls.outbox;

  await user.click(screen.getByTestId('export-xlsx'));

  await vi.waitFor(() => expect(createObjectURL).toHaveBeenCalled());
  expect(clickSpy).toHaveBeenCalled();
  expect(calls.outbox).toBe(outboxCallsBefore); // the export never re-fetches the list
  expect(exportUrl!.searchParams.get('status')).toBe('dead');
...
test('the dead-letter Excel button asks the server for the export with the applied filter, never paging the list itself', async () => {
  mockBackend({ outbox: [outboxMessage()], letters: [deadLetter()] });
  let exportUrl: URL | null = null;
  server.use(
    http.get('*/api/v1/admin/integrations/dead-letters/export.xlsx', ({ request }) => {
      exportUrl = new URL(request.url);
      return HttpResponse.text('xlsx-bytes', {
        headers: {
          'Content-Disposition': 'attachment; filename="kiruvchi-xatolar-2026-09-11.xlsx"',
          'X-Export-Total': '1',
          'X-Export-Rows': '1',
          'X-Export-Truncated': 'false',
        },
  ...
  await screen.findByTestId(`outbox-row-${DEAD_MESSAGE}`);
  await user.click(screen.getByRole('button', { name: /Kiruvchi xatolar/ }));
  await screen.findByTestId(`letter-row-${NEW_LETTER}`);
  await user.selectOptions(screen.getByTestId('letters-status-filter'), 'new');
  await user.click(screen.getByRole('button', { name: 'Qoʻllash' }));
  await screen.findByTestId(`letter-row-${NEW_LETTER}`);
  const lettersCallsBefore = calls.letters;

  await user.click(screen.getByTestId('export-xlsx'));

  await vi.waitFor(() => expect(createObjectURL).toHaveBeenCalled());
  expect(clickSpy).toHaveBeenCalled();
  expect(calls.letters).toBe(lettersCallsBefore); // the export never re-fetches the list
  expect(exportUrl!.searchParams.get('status')).toBe('new');
  expect(exportUrl!.searchParams.get('lang')).toBe('uz_latn');
  expect(exportUrl!.searchParams.has('page')).toBe(false);
  expect(exportUrl!.searchParams.has('page_size')).toBe(false);
