...
export function DashboardCard({
  ...
  return (
    <section className={`bg-white border border-[#E4E7EA] rounded-2xl p-5 sm:p-6 shadow-xs ${className}`}>
      <header className="flex flex-wrap items-start justify-between gap-3 mb-5">
        <div className="flex items-start gap-2.5 min-w-0">
          {Icon ? <Icon className="w-5 h-5 text-[#2E7D4F] shrink-0 mt-0.5" /> : null}
          <div className="min-w-0">
            <h2 className="text-base font-bold text-[#1A1F24] leading-snug">{title}</h2>
            {subtitle ? <p className="text-xs text-[#5A646D] mt-1 leading-relaxed">{subtitle}</p> : null}
          </div>
        </div>
        {badge}
      </header>
