...
test('the Excel button asks the server for the export with the applied status filter, never paging the list itself', async () => {
  ...
  server.use(
    ...
    http.get('*/api/v1/gis/imports/export.xlsx', ({ request }) => {
      exportCalls += 1;
      exportUrl = new URL(request.url);
      return HttpResponse.text('xlsx-bytes', {
        headers: {
          'Content-Disposition': 'attachment; filename="importlar-2026-09-11.xlsx"',
          'X-Export-Total': '0',
          'X-Export-Rows': '0',
  ...
  await user.selectOptions(
    within(screen.getByTestId('imports-list-panel')).getByRole('combobox'),
    'failed',
  );
  ...
  expect(exportUrl!.searchParams.get('status')).toBe('failed');
