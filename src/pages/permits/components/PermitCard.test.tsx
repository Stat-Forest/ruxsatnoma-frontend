...
test('a click anywhere on the card opens the permit, not only its button', async () => {
  renderCard();
  await userEvent.setup().click(screen.getByRole('heading', { level: 3 }));
  expect(screen.getByTestId('current-location')).toHaveTextContent(`/my/permits/${PERMIT_ID}`);
});

test('the "view" control is still a real link', () => {
  renderCard();
  expect(screen.getByRole('link')).toHaveAttribute('href', `/my/permits/${PERMIT_ID}`);
});
